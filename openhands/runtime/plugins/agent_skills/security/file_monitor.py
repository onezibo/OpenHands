...
class FileSystemMonitor:
    ...
    def add_watch(self, 
        ...
        try:
                        ...
                        if recursive:
                            for subpath in path.rglob('*'):
                                if subpath.is_dir():
                                    ...
                                    sub_wd = self._inotify.add_watch(str(subpath), flags)
            ...
            logger.info(f"添加文件监控: {path} (键: {watch_key}, 模式: {'轮询' if self.use_polling else 'inotify'})")
            return True
        ...
        try:
            with self._event_lock:
                ...
                if self.use_polling:
                    ...
                    # 移除递归监控的子目录
                    keys_to_remove = [k for k in self._watch_descriptors.keys() 
    ...
    def _check_polling_changes(self, watch_key: str):
        ...
        for file_path, mtime in current_state.items():
            if file_path not in old_state:
                # 新文件
                self._emit_event(watch_key, FileEvent(
    ...
    def _handle_inotify_event(self, event):
        """处理inotify事件"""
        try:
            event_path = Path(event.path) / event.name if event.name else Path(event.path)
            ...
            if event.mask & inotify_simple.flags.CREATE:
                ...
                event_type = FileEventType.MODIFIED
            elif event.mask & (inotify_simple.flags.DELETE | inotify_simple.flags.DELETE_SELF):
                event_type = FileEventType.DELETED
            ...
            file_event = FileEvent(
                ...
                timestamp=time.time(),
                size=event_path.stat().st_size if event_path.exists() and event_path.is_file() else None,
    ...
    def get_monitoring_status(self) -> Dict[str, any]:
        ...
        return {
            ...
            'watched_paths': len(self._polling_paths if self.use_polling else self._watch_descriptors),
...
class AFLCrashMonitor:
    ...
    def __init__(self, crashes_dir: str, on_crash_found: Callable[[List[Path]], None] = None):
        """初始化AFL崩溃监控器
    ...
    def start_monitoring(self) -> bool:
        ...
        try:
            ...
            if success:
                self._monitoring = True
                logger.info(f"AFL崩溃监控启动成功，当前崩溃文件: {len(self._crash_files)}个")
                return True
            else:
    ...
    def _handle_file_event(self, event: FileEvent):
        ...
        try:
            # 只关心crashes目录中的新文件
            if (event.event_type == FileEventType.CREATED and 
                not event.is_directory and
                str(event.file_path).startswith(str(self.crashes_dir))):
                
                # 检查是否是崩溃文件（以id:开头，不是README文件）
                if (event.file_path.name.startswith('id:') and 
                    'README' not in event.file_path.name):
