...
def compile_for_klee(
    ...
    cmd_parts.extend([
        ...
        "-Xclang", "-disable-O0-optnone"  # 确保KLEE可以分析
    ])
        ...
        try:
            with open(run_stats_file, 'r') as f:
                ...
                if len(lines) > 1:
                    ...
                    # 显示关键指标
                    key_metrics = ['Instructions', 'FullBranches', 'PartialBranches', 
    ...
    if os.path.exists(cov_file):
        ...
        try:
            with open(cov_file, 'r') as f:
                ...
                if len(lines) > 1:
                    # 简单统计覆盖的指令数
                    covered_instructions = len([line for line in lines[1:] if line.strip()])
            ...
            if result.returncode == 0:
                ...
                for line in output_lines:
                    ...
                    if line.startswith('object'):
                        ...
                        current_object['name'] = line.split(':', 1)[1].strip().strip('"')
                ...
                for obj in objects:
                    if obj['name'] and obj['size'] > 0:
                        results.append(f"  对象: {obj['name']}, 大小: {obj['size']} bytes")
                        if obj['data']:
                            # 显示数据的可读形式
                ...
                if save_to_dir:
                    ...
                    # 如果是简单的输入数据，也创建二进制文件
                    if objects and len(objects) == 1 and objects[0]['name'] in ['stdin', 'arg']:
                        binary_file = os.path.join(save_to_dir, f"input_{i+1:03d}.bin")
                        try:
                            # 简单的十六进制转换（ktest-tool输出格式）
                            hex_data = objects[0]['data'].replace('\\\\x', '').replace(' ', '')
                            if all(c in '0123456789abcdefABCDEF' for c in hex_data):
                                binary_data = bytes.fromhex(hex_data)
                                with open(binary_file, 'wb') as bf:
                                    bf.write(binary_data)
                                results.append(f"  已保存二进制输入: {os.path.basename(binary_file)}")
                        except Exception:
                ...
                results.append(f"测试用例 {i+1}: 解析失败 - {os.path.basename(ktest_file)}")
                if result.stderr:
...
def check_klee_status(output_dir: str) -> str:
    ...
    status_info = []
    status_info.append(f"=== KLEE执行状态 ===")
    status_info.append(f"输出目录: {output_dir}")
    
    # 检查是否有进程在运行
    try:
        result = subprocess.run("pgrep -f klee", shell=True, capture_output=True, text=True)
        if result.returncode == 0:
