...
def batch_analyze_crashes(
    ...
    for i, crash_file in enumerate(crash_files, 1):
        ...
        if output_dir:
            ...
            analysis_file = os.path.join(output_dir, f"{os.path.basename(crash_file)}.analysis")
    ...
    for signal, crashes in signal_groups.items():
        ...
        for crash in crashes[:5]:  # 每组最多显示5个
            results.append(f"  - {crash['file']}: {crash['info'].get('crash_function', 'Unknown')}")
        if len(crashes) > 5:
...
def extract_crash_info(gdb_output: str) -> Dict[str, str]:
    ...
    # 也检查明显的非法地址
    if any(addr in info['crash_address'].lower() for addr in ['41414141', '42424242', '43434343']):
        info['control_hijack'] = True
...
def check_exploitability(
    ...
    try:
        ...
        result = subprocess.run(exploitable_cmd, shell=True, capture_output=True, text=True)
        
        if "exploitable" in result.stdout.lower() and "not exploitable" not in result.stdout.lower():
...
def generate_crash_report(
    ...
    report_lines.append(f"生成时间: {subprocess.run('date', shell=True, capture_output=True, text=True).stdout.strip()}")
    report_lines.append("")
    
    # 统计信息
    crash_stats = {'total': len(crash_files), 'analyzed': 0, 'exploitable': 0, 'high_severity': 0}
    ...
    for i, crash_file in enumerate(crash_files):
        ...
        report_lines.append(f"控制流劫持: {'是' if crash_info['control_hijack'] else '否'}")
        report_lines.append(f"栈溢出: {'是' if crash_info['stack_overflow'] else '否'}")
        report_lines.append(f"堆损坏: {'是' if crash_info['heap_corruption'] else '否'}")
    ...
    report_lines.append(f"风险评级: {'高' if crash_stats['exploitable'] > 0 else '中' if crash_stats['high_severity'] > 0 else '低'}")
