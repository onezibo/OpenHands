...
class TestSecuritySkills(unittest.TestCase):
    ...
    def test_check_binary_security_import(self, mock_run):
        ...
        try:
            from openhands.runtime.plugins.agent_skills.security.analysis_skills import check_binary_security
    ...
    def test_afl_skills_import(self, mock_run):
        """测试AFL++ skills的导入"""
        try:
            from openhands.runtime.plugins.agent_skills.security.afl_skills import start_fuzzing
            
            # 模拟文件不存在的情况
            result = start_fuzzing("/nonexistent/binary", "/nonexistent/input", "/tmp/output")
    ...
    def test_gdb_skills_import(self):
        """测试GDB skills的导入"""
        try:
            from openhands.runtime.plugins.agent_skills.security.gdb_skills import analyze_crash
            from openhands.runtime.plugins.agent_skills.security.gdb_skills import extract_crash_info
    ...
    def test_klee_skills_import(self):
        """测试KLEE skills的导入"""
        try:
            from openhands.runtime.plugins.agent_skills.security.klee_skills import compile_for_klee
    ...
    def test_analysis_skills_workspace_creation(self):
        """测试分析工作空间创建"""
        try:
            from openhands.runtime.plugins.agent_skills.security.analysis_skills import create_analysis_workspace
            ...
            self.assertTrue(os.path.exists(os.path.join(workspace_dir, "fuzzing", "input")))
...
class TestSecurityAgentIntegration(unittest.TestCase):
    ...
    def test_microagents_available(self):
        ...
        for microagent_file in microagent_files:
            ...
            self.assertTrue(
                os.path.exists(microagent_path), 
                f"微代理文件不存在: {microagent_file}"
            )
    ...
    def test_docker_configuration_available(self):
        ...
        for docker_file in docker_files:
            ...
            self.assertTrue(
                os.path.exists(docker_path),
                f"Docker配置文件不存在: {docker_file}"
            )
...
class TestSecurityAgentPerformance(unittest.TestCase):
    ...
    def test_agent_initialization_time(self):
        ...
        start_time = time.time()
        agent = SecurityAgent(mock_llm, config)
