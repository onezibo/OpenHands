...
class TestCVEReproductionWorkflow(unittest.TestCase):
    ...
    def test_nvd_browser_tool_usage(self):
        ...
        with patch('requests.get') as mock_get:
            ...
            exploit_pattern = (
                r'<span class="label label-danger">Exploit</span>\s*<a href="([^"]+)"'
            )
    ...
    def test_afl_integration_for_cve(self):
        ...
        for scenario in afl_usage_scenarios:
            self.assertTrue(
                len(scenario) > 0, f'AFL++使用场景 {scenario} 应该有具体的实现'
            )
...
class TestCVEReproductionEffectiveness(unittest.TestCase):
    ...
    def test_agent_browser_tool_strategy(self):
        ...
        for strategy_name, strategy_desc in browser_tool_strategy.items():
            self.assertIn(
                '分析', strategy_desc, f'策略 {strategy_name} 应该包含分析步骤'
            )
            self.assertTrue(
                len(strategy_desc) > 10, f'策略 {strategy_name} 应该有详细的描述'
            )
...
def run_cve_reproduction_tests():
    ...
    # 添加测试用例
    test_suite.addTest(
        unittest.TestLoader().loadTestsFromTestCase(TestCVEReproductionWorkflow)
    )
    test_suite.addTest(
        unittest.TestLoader().loadTestsFromTestCase(TestCVEReproductionEffectiveness)
    )
